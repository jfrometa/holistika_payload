      ...
      <div ref={innerRef} className="inner">
        ...
        <ul className="mainmenu">
          {menuItems.map((item, index) => (
            <li
              key={index}
              className={`${item.hasMegamenu ? 'with-megamenu has-menu-child-item ' : ''} ${
                item.hasDropdown ? 'has-droupdown has-menu-child-item' : ''
              } ${item.megamenuClass == 'with-mega-item-2' ? 'position-relative' : ''}`}
            >
              {item.href ? (
                <Link
                  className={pathname.split('/')[1] == item.href.split('/')[1] ? 'active' : ''}
                  href={item.href}
                >
                  {item.title}
                </Link>
                ...
                >
                  <span className={isActiveParent(item) ? 'activeParent' : ''}>{item.title}</span>
                </a>
              )}
              {item.hasMegamenu && (
                            ...
                            {column.map((link, linkIndex) => (
                              <li key={linkIndex}>
                                ...
                                >
                                  {link.label}{' '}
                                  {link.badge && (
                                    <span className="rainbow-badge-card">{link.badge}</span>
                                  )}
                                </Link>
                              </li>
                            ))}
                ...
                <ul className="submenu">
                  {item.submenu.map((submenuItem, submenuIndex) => (
                    <li key={submenuIndex}>
                      <Link
                        className={
                          pathname.split('/')[1] == submenuItem.href.split('/')[1] ? 'active' : ''
                        }
